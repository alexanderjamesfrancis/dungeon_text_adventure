...
def player_builder():
    ...
    print(f'{name} eh? Hopefully it is a name that will be venerable to ones house.\n')

    # age input

    age = int(input("How many Summers have you experienced?\n"))
    if age <= 28:
        agility += 2
    ...
    while (build != "skinny") and (build != "typical") and (build != "muscular"):
        build = input('What physical build are you: Skinny, Typical, Muscular?\n')
    ...
    print(f'The Baron leans back in his chair. "Now {name}, that I am satisfied that you are physically capable, I would like to know more about your background. First however, can I offer you a drink?\n')

    # tea offer

    drink = input("Will you take the Baron's offer? Yes or No?\n").lower()
    if drink.lower() != "yes" and "no":
        drink = input("Will you take the Baron's offer? Yes or No?\n")
